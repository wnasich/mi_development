...
$this->templateVars['postActions'][] = $admin . 'move_down';
?>
	public function <?php echo $admin ?>move_down($id = null, $steps = 1) {
		$this-><?php echo $currentModelName ?>->id = $id;
		if ($id && $this-><?php echo $currentModelName ?>->exists()) {
			$display = $this-><?php echo $currentModelName ?>->display($id);
			if (!$this-><?php echo $currentModelName ?>->moveDown($id, $steps)) {
